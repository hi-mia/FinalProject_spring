...
public void makePagingHTML() {
		...
		if(startPage>pageBlock)
			...
			pagingHTML.append("<span id='prev_paging' onclick='managerPaging("+(startPage-1)+")'>이전</span>");
		
		for(int i=startPage; i<=endPage; i++) {
			
			if(i==currentPage) {
				//pagingHTML.append("[  <a id='currentPaging' href='imageboardList.jsp?pg="+i+"'>"+i+"</a>  ]");
				pagingHTML.append("<span id='currentPaging' name='currPageValue' onclick='managerPaging("+i+")'>"+i+"</span>");
			}else {
				//pagingHTML.append("[  <a id='paging' href='imageboardList.jsp?pg="+i+"'>"+i+"</a>  ] ");
				pagingHTML.append("<span id='paging' name='notCurrPageValue' onclick='managerPaging("+i+")'>"+i+"</span>");
				}
		}//for
		
		if(endPage< totalP)
			//pagingHTML.append("<a id='paging' href='imageboardList.jsp?pg="+(endPage+1)+"'>[다음]</a>");
			pagingHTML.append("<span id='next_paging' name='back' onclick='managerPaging("+(endPage+1)+")'>다음</span>");
