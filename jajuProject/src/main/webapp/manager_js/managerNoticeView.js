...
$(function(){
	$.ajax({
		type: 'post',
		url: '/jaju/manager/getNoticePage',
		data: {'notice_seq' : $('#notice_seq').val()},
		dataType: 'json',
		success: function(data){
			//alert(JSON.stringify(data));
			...
			if(data.noticeDTO.prev_subject != '이전글이 없습니다'){
				$('#prevBtn').attr('href', '/jaju/manager/managerNoticeView?notice_seq='+data.noticeDTO.prev_num+'&pg=1');
			}
			
			$('#nextSpan').text(data.noticeDTO.next_subject);
			if(data.noticeDTO.next_subject != '다음글이 없습니다'){
				$('#nextBtn').attr('href', '/jaju/manager/managerNoticeView?notice_seq='+data.noticeDTO.next_num+'&pg=1');
			}
